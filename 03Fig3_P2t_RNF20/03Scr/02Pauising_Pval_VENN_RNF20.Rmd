---
title: "02Pauising_Pval_VENN_RNF20"
author:
...
PROJECT="02Pauising_Pval_VENN_RNF20"
PTHA11="../03OUTPUT/"
dir.create(PTHA11)
...
meta2 = "../01DATA/01Norma_countPI_RNF20/"
...
c2 = "../01DATA/length_1kbmorev45.basic.bed"
cat("Current file name is:",c2,"\n")
...
MAT_AA2$PI_R20_mean<- rowMeans(MAT_AA2[,5:7])
MAT_AA2$diffPI<- MAT_AA2$PI_R20_mean-MAT_AA2$PI_Ctr_mean
...
```{r}
WORK1 = "../01DATA/02Norma_countPI_fmer/HUVEC_P2t_siCtrl_tag_count_normalize.txt"

 mat_c = read.delim(WORK1,header=T,sep="\t",dec=".")
 WORK22 = "../01DATA/02Norma_countPI_fmer/HUVEC_P2t_siRNF20_tag_count_normalize.txt"
...
library(rstatix)
library(ggpubr)
mat_RPKM1_5_c <- gather(MAT_AA3, colnames(MAT_AA3[,2:7]), key="Sample",value= "log2fc")

G2<- data.frame(str_split_fixed(mat_RPKM1_5_c$Sample,"_", 8))
G2$Sample2<- paste(G2$X3,G2$X5,sep="_")
mat_RPKM1_5_c1<- cbind(mat_RPKM1_5_c,G2$Sample2)
...
head(GN5)
MAT_AA3$PI_Ctr_mean<- rowMeans(MAT_AA3[,2:4])
MAT_AA3$PI_R20_mean<- rowMeans(MAT_AA3[,5:7])
MAT_AA3$PI<- (MAT_AA3$PI_R20_mean+0.1)/(MAT_AA3$PI_Ctr_mean+0.2)
MAT_AA3$log2FCPI_R20_v_Ctr<- log2(MAT_AA3$PI)
...
LOGUP=0
PVALUE=0.05
resdk4m3_down <- subset(MAT_AA4, p < PVALUE & log2FCPI_R20_v_Ctr <LOGD)
DWn<-  length(unique(resdk4m3_down$SYMBOL))
   CHECK3<-  subset(resdk4m3_down,SYMBOL=="DLL4" | SYMBOL=="NRARP"| SYMBOL=="JUNB")

##UP log2FC >= 1.5
resdk4m3_up <- subset(MAT_AA4, p < PVALUE & log2FCPI_R20_v_Ctr > LOGUP)
...
MAT_AA4$log2PI<- MAT_AA4$log2FCPI_R20_v_Ctr
...
ELONG <-subset(MAT_AA4, p.adj < PVALUE & log2FCPI_R20_v_Ctr <LOGD)
PAUS <-subset(MAT_AA4,  p.adj < PVALUE &log2FCPI_R20_v_Ctr > LOGUP)
ELONG$GROUP<- "aLOSS"
PAUS$GROUP<- "bGAIN"
df2Rnf40_diff_RNA_u <-rbind(ELONG,PAUS)
...
TSS1=-1000
TSS2=1000
dBGI06_2 = "../01DATA/03Published_CHIP_seq/01ERG_GSE124893_vs_INP_MACS2_f001_EX150_annot"
...
library("readxl")
EST1_2 = "../01DATA/03Published_CHIP_seq/02EST1_12h_GSE93030_v_INP_MACS2_f001_EX150_vs_bl_annot"
...
CP1vsRnf40 <- list(mat_dBGI06_2b_1,mat_EST1_2b_1,df2Rnf40_diff_RNA_u) %>%
...
venn.plot <- venn.diagram(
  list(PAU_LOSS= ELONG$SYMBOL, ERG_PE = mat_dBGI06_2b_1$SYMBOL, EST1_PE = mat_EST1_2b_1$SYMBOL), 
  D0,
  col = "transparent",
  fill = c( "#f4cae4","#ef8a62","darkgrey"),
  alpha = TT,
  label.col = c("black",  "black", "black", "black", "white","black", "black" ),
  cex = 1,
  fontfamily = "serif",
  fontface = "bold",
  cat.col = c("#ef8a62", "#ef8a62", "black"),
  cat.cex = 0.75,
  cat.pos = 0,
  cat.dist = LL,
...
require(VennDiagram)
venn.plot <- venn.diagram(
  list(PAU_GAIN = PAUS$SYMBOL, ERG_PE = mat_dBGI06_2b_1$SYMBOL, EST1_PE = mat_EST1_2b_1$SYMBOL), 
  D2,
  col = "transparent",
  fill = c(upcol,"#ef8a62","darkgrey"),
  alpha = TT,
  label.col = c("black",  "black", "black", "black", "white","black", "black"),
  cex = 1.5,
  fontfamily = "serif",
...
MA1<- subset(df2Rnf40_diff_RNA_u,select=c("SYMBOL","GROUP"))
MA2<- rbind(MA1,mat_dBGI06_2b_1,mat_EST1_2b_1)
MA2$freq=1
MA2_u<- MA2[!duplicated(MA2[,c("SYMBOL","GROUP")]),]

GN6<- spread(MA2_u,GROUP,freq,fill=0)
colnames(GN6)<- c("SYMBOL","aLOSS","bGAIN","ERG_PEAKS","EST1_PEAKS_12h")
length(GN6$SYMBOL)
...
MA6<- plyr::count(GN6,c("aLOSS","bGAIN", "ERG_PEAKS","EST1_PEAKS_12h"))
write.xlsx(MA6,file=paste(PTHA,"010_",PROJECT, "_forPIE.xlsx",sep=""))
